def landing_gear_sizing():
    #ground_clearance, MTOW, wheel_diameter, nlg_distance_from_cg, mlg_distance_from_cg, sink_speed
    ...
    lm = 1.5 # Distance between CG and main gear
    ln = 8 # Distance between CG and nose gear
    ...
    for load_case in range(3):
        ...
        def compute_nlg_forces():
            nlg_axial_force = MTOW * (lm + y_cg * ax_over_g)/(lm + ln) # axial force for the nose isn't affected by load cases as there is only one nose gear
