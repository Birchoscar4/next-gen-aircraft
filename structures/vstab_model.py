def compute_vstab_mass(spar_cap_radius, num_stringers, skin_thickness, spar_thickness):
    #LE_spar, TE_spar, v, spar_cap_radius, num_stringers, skin_thickness, spar_thickness, load_factor
    import numpy as np
    import re
    import pandas as pd
    ...
    file_path = "Poster_Showcase_Model_VertStab_dCp.txt"
    aerofoil_file = "n0012.dat"
    rho = 0.36518 # kg/m3 - air
    v = 210 # m/s
    q_inf = 0.5 * rho * v **2
    LE_spar = 0.1
    TE_spar = 0.7
    ...
    def compute_lift_forces(file_path, q_inf):
        ...
        with open(file_path, "r", errors="ignore") as file:
            for line in file:
                match = re.match(r"BLOCK Cut_\d+_at_Z:_([\d\.\-]+)", line)
                if match:
                    current_slice = float(match.group(1))  # Use Z as the slice position
                ...
                if len(values) == 4:
                    try:
                        ...
                        if y != 7.0:
                            continue  # Only use the +7 y side
                        slices[current_slice].append((x, dCp))
                    except ValueError:
                        continue

        lift_slices = []
        for z_slice, data in slices.items():
            if len(data) < 2:
                continue
    ...
    def extract_wing_geometry(file_path):
        ...
        with open(file_path, "r", errors="ignore") as file:
            for line in file:
                match = re.match(r"BLOCK Cut_\d+_at_Z:_([\d\.\-]+)", line)
                if match:
                    current_slice = float(match.group(1))  # Use Z as the spanwise coordinate
                    slices[current_slice] = []
                    continue
                ...
                if len(values) == 4:
                    try:
                        x, y, z, dCp = map(float, values)
                        if y != 7.0:  # Keep only +7 y-side values
                            continue
                        slices[current_slice].append(x)  # Only x is needed
                    except ValueError:
                        continue
        ...
        z_root, z_tip = z_values[0], z_values[-1]
        span = z_tip - z_root

        x_root = np.array(slices[z_root])
        x_tip = np.array(slices[z_tip])
        x_LE_root, x_TE_root = np.min(x_root), np.max(x_root)
        x_LE_tip, x_TE_tip = np.min(x_tip), np.max(x_tip)

        c_root = x_TE_root - x_LE_root
        c_tip = x_TE_tip - x_LE_tip
        ...
        delta_z = z_tip - z_root
        sweep_quarter_chord = np.arctan(delta_x_quarter / delta_z) if delta_z != 0 else np.nan
        ...
        return {
            ...
            "Outerboard Span": span,
            "x_root": x_quarter_root,
            "x_tip": x_quarter_tip,
            "y_root": z_root,  # now corresponds to Z
            "y_tip": z_tip
        }
    ...
    def find_spar_heights(aerofoil, LE_spar, TE_spar):
        ...
        rear_spar_y = aerofoil[np.abs(aerofoil[:, 0] - rear_spar_x).argmin()+1][1] - aerofoil[np.abs(aerofoil[:, 0] - rear_spar_x).argmin()][1]
    ...
    def calculate_twisting_moment(aerodynamic_loads, CoPs, wingbox_chord, LE_spar, wingbox_fraction, xshear):
    ...
    def wingbox_mass_inertia(density, root_chord, taper_ratio, y_root, y_tip, t_over_c, skin_thickness, spar_thickness, A_spar_cap, A_stringer, num_stringers, sweep_angle, LE_spar, TE_spar, wingbox_fraction):
        ...
        z_cg = 7 # wing height above nose - can be defined elsewhere in the system
    ...
    for load_case in range(2):
        ...
        for i in range(0, elements):
            ...
            EI[i] = E * I[i]
            J[i] = wingbox_polar_moment(spar_thickness=spar_thickness, wingbox_height=wingbox_height[i],skin_thickness=skin_thickness,wingbox_chord=wingbox_chord[i])
            GJ[i] = G * J[i]
        ...
        Q = calculate_twisting_moment(aerodynamic_loads, CoPs, wingbox_chord, LE_spar, wingbox_fraction, xshear) 
        #plt.plot(z,Q)
    ...
    """    plt.figure(figsize=(12, 5))
        ...
        plt.show()"""
    print(buckled)
    return [x_cg,z_cg,y_cg,I_x,I_z,I_y,mass,buckled]

#compute_vstab_mass(0.02,8,0.001,0.01)