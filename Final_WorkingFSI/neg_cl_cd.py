def neg_cl_cd():    
    ...
    # Execute
    print("\tExecuting Negative Deflection Analysis")
    ...
    Mach = 0.3
    if Mach < 0.8:
        df = pd.read_csv('C:/Users/joepe/Documents/ProjectAero/OpenVSP-3.41.1-win64-Python3.11/OpenVSP-3.41.1-win64/python/update_geom_neg_DegenGeom.polar', delim_whitespace=True)
        ...
        df.loc[df['AoA'] < 0, 'CDtot'] = 1.0718*(df.loc[df['AoA'] < 0, 'CDtot'] + 0.016)

        df['AoA_Squared'] = df['AoA'] ** 2

        df.loc[df['AoA'] > 0, 'CDtot'] = 1.0718*((df.loc[df['AoA'] > 0, 'AoA_Squared']*0.0018) + (df.loc[df['AoA'] > 0, 'AoA']*0.004) + df.loc[df['AoA'] > 0, 'CDtot'] + 0.02)
        df.loc[df['AoA'] == 0, 'CDtot'] = 1.0718*((df.loc[df['AoA'] == 0, 'AoA_Squared']*0.0018) + (df.loc[df['AoA'] == 0, 'AoA']*0.004) + df.loc[df['AoA'] == 0, 'CDtot'] + 0.02)
        print(df.head()) 
        ...
        plt.figure(figsize=(8, 6))
        plt.scatter(df['AoA'], df['CMy'], color='blue', alpha=0.5)
        plt.title("Plot of AoA vs CMy")
        plt.xlabel("AoA")
        plt.ylabel("CL")
        plt.grid(True)
        plt.show() 
        ...
        df.loc[df['AoA'] > 2, 'CDtot'] = 1.0718*(df.loc[df['AoA'] > 2, 'CDtot'] - 0.0472)

        df['AoA_Squared'] = df['AoA'] ** 2

        df.loc[df['AoA'] < 2, 'CDtot'] = 1.0718*((df.loc[df['AoA'] > 0, 'AoA_Squared']*0.0006) + (df.loc[df['AoA'] > 0, 'AoA']*0.001) + (df.loc[df['AoA'] > 0, 'CDtot']*0.0142) + 0.0138)
        df.loc[df['AoA'] == 2, 'CDtot'] = 1.0718*(df.loc[df['AoA'] == 2, 'CDtot'] - 0.0472)
        print(df.head()) 
