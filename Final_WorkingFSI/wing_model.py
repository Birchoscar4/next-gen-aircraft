def compute_wing_mass(spar_cap_radius, num_stringers, skin_thickness, spar_thickness, max_q_velocity, max_q_density, load_factors, LE_spar, TE_spar, file_path):
    ...
    return [x_cg,y_cg,z_cg,I_x,I_y,I_z,mass,np.max(buckled), tip_deflection[0], tip_deflection[1], np.max(abs(tip_twist)), TE_spar, Ve, alpha_e]

#compute_wing_mass(0.08, 8, 0.01, 0.02, 210, 0.38, [3.5,-1], 0.1, 0.7,"Prelim_3_DegenGeom.slc")
